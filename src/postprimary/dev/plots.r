...
if (F) {
  setwd("X:/p4/acq/Sequel/ppa/dev")
  filename = "m54011_160306_050740.csv"
...
d_orig <- read.csv(file=filename, header=TRUE, sep=",")
d_orig$minSnr <- pmin( d_orig$hqRegionSnr0,d_orig$hqRegionSnr1,d_orig$hqRegionSnr2,d_orig$hqRegionSnr3 )
d_orig$maxSnr <- pmax( d_orig$hqRegionSnr0,d_orig$hqRegionSnr1,d_orig$hqRegionSnr2,d_orig$hqRegionSnr3 )
d_orig$meanSnr <- ( d_orig$hqRegionSnr0 + d_orig$hqRegionSnr1 + d_orig$hqRegionSnr2 + d_orig$hqRegionSnr3 ) / 4
# d_orig$medianSnr <- pmedian( d_orig$hqRegionSnr0,d_orig$hqRegionSnr1,d_orig$hqRegionSnr2,d_orig$hqRegionSnr3 )
d_orig$qSize  <- d_orig$qEnd -d_orig$qStart
d_orig$aSize  <- d_orig$aEnd -d_orig$aStart
d_orig$hqSize <- d_orig$hqEnd-d_orig$hqStart
head(d_orig)
minSnr <- 0
d <- d_orig[(d_orig$minSnr >= minSnr) | is.na(d_orig$hqRegionSnr0),]

d$snrBin = floor(d$minSnr)
d$class  = ifelse(is.na(d$qStart),2,0) + ifelse(is.na(d$hqStart),1,0)
...
plot.new()
field <- c("csvfile","host","dir","rerundir","num ZMWs","reference","comment")
value <- c(filename , Sys.getenv("HOSTNAME"), getwd(), Sys.getenv("rerundir"), nrow(d), Sys.getenv("reference"),Sys.getenv("COMMENT"))
...
map_hq     <- nrow(d[!is.na(d$qStart) & !is.na(d$hqStart),])
map_nohq   <- nrow(d[!is.na(d$qStart) &  is.na(d$hqStart),])
nomap_hq   <- nrow(d[ is.na(d$qStart) & !is.na(d$hqStart),])
nomap_nohq <- nrow(d[ is.na(d$qStart) &  is.na(d$hqStart),])
...
p1 <- ggplot(d[ is.na(d$hqStart),],aes(subreads)) + geom_histogram(color="black",fill="white") + 
  scale_y_log10() +
  ggtitle("Mapped subreads per ZMW\nwith no HQ")
p2 <- ggplot(d[ is.na(d$hqStart),],aes(aSize)) + geom_histogram(color="black",fill="white",binwidth=100) + 
  ggtitle("subread length\nwith no HQ")

d_nohq <- d[ is.na(d$hqStart),]
...
p3 <- ggplot(d[ !is.na(d$hqStart),],aes(subreads)) + geom_histogram(color="black",fill="white") + 
  scale_y_log10() +
  ggtitle("Mapped subreads per ZMW\nwith HQ")
p4 <- ggplot(d[ !is.na(d$hqStart),],aes(aSize)) + geom_histogram(color="black",fill="white",binwidth=100) + 
  ggtitle("subread length\nwith HQ")

d_whq <- d[! is.na(d$hqStart) & !is.na(d$aSize),]
...
d1$overlap <- ! ( d1$aEnd < d1$hqStart || d1$hqEnd < d1$aStart)

d1$hqadv <- d1$hqStart - d1$aStart
d1$hqret <- d1$hqEnd - d1$aEnd
...
print("xx")
head(xx)
locations <- data.frame(d0_big$holeNumber,xx,yy,round(d0_big$minSnr,1),d0_big$hqStart)
plot.new()
